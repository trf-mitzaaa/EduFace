...
while True:
        ...
        for nume, timp in list(timp_prezenti.items()):
            if (acum - timp) > durata_prezenta:
                ...
                if match:
                    ...
                    try:
                        cursor.execute("""
                            UPDATE attendance_current SET present=0 WHERE student_id = %s
                        """, (student_id,))
                        conn.commit()
    ...
    if current_unknown and (datetime.now() - last_unknown_face_time).total_seconds() > 5:
    ...
    cv2.rectangle(frame, (10, 10), (500, 40), (255, 255, 255), -1)
    cv2.putText(frame, "1 = Iesire | 2 = Afiseaza/ascunde jurnal | 3 = Schimba camera", (20, 30),
